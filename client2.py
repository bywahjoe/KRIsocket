...
def otomatis():
    ...
    while True:
        ...
        # ngejar bola jika jarak kurang dari X
        elif STEP_ROBOT == 2:
            print('STEP_ROBOT = 2')

            is_ball_close = jarak_ball <= 130

            if is_bolo_found and is_ball_catch is False:
                if ballX > 300 and ballX < 330:
        ...
        # madep bolo karo ngenteni komando umpan
        elif STEP_ROBOT == 3:
            print('STEP_ROBOT = 2')

            # * madep bolo'
            if (boloX > 300 and boloX < 330):
                # lek wes maded bolo tendang
                # ngirim ping
                time.sleep(3)
                tendang()
                stop()
